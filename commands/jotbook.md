---
description: Guided jotbook curation flow — review the staged backlog, then bind the survivors. The primary entry point.
---

# /jotbook

Guided curation flow. Bundles **review** and **bind** into a single pass for clearing the backlog.

If args are passed to `/jotbook`, ignore them and gently point the user at `/jot` for granular actions (e.g., "for individual actions, use `/jot`, `/jot review`, `/jot bind <slug>`"). Then proceed with the flow below.
...
2. Let the user respond with their decisions (drop / merge / tweak / bind / keep).
3. Apply the structural decisions (drops, merges, tweaks) via `jotbook-review`.
4. For each jot the user marked as ready to **bind**, invoke the `jotbook-bind` skill on that slug. If they marked several, ask whether to do them one-by-one with checkpoints, or batch through them.
5. End with a one-line summary of what was dropped, merged, tweaked, and bound.

If the backlog is empty, say so in one line and stop — do not proceed to the bind step.

## Hard rules

- Do not silently invent slugs or jots that don't exist on disk.
- Do not call `jotbook-bind` without the user explicitly marking a jot as ready.
