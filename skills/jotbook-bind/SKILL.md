---
name: jotbook-bind
description: Bind one or more reviewed jots into a finished long-form entry in the jotbook. Reads the referenced source files, optionally consolidates multiple jots, and writes the entry under the configured entries directory. Removes consumed jots on success. Invoked by `/jot bind <slug>[,<slug>]` or as the final phase of the bare `/jotbook` flow.
---

# Bind a jot into the jotbook

You're turning a jot (or a small set of related jots) into a long-form entry. The jot file is a *pointer*; the actual content needs to be reconstructed from the referenced files, conversation history, and — if context is thin — direct questions to the user. The output is a bound entry in the jotbook.
...
> "I can't bind this entry — `output_format` is `html` but `template_path` isn't set (or points to a missing file). Two ways forward: (1) switch `output_format` to `markdown` in your settings and re-run, or (2) start a separate conversation to design the HTML template — Claude can help you build one, but it's a substantial design task that shouldn't happen inside the `/jot bind` flow. Once the template exists, point `template_path` at it and re-run."

Then stop. The point is to make the template a deliberate, named artifact rather than something improvised mid-bind.
...
If this entry deserves to be linked **from** an existing entry, add a link from the appropriate place in that entry. Confirm with the user before editing a previously-bound entry.
...
- **Don't proliferate output files.** One entry per bind, unless the user explicitly asks for a multi-entry set.
