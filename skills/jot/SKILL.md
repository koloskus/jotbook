...
- **`link`** → invoke the `jotbook-link` skill (the cross-link sweep over inked entries). Pass any remaining text as an optional focus slug to scope the sweep to one entry's connections.
